...
window.Multiplayer = {
    ...
    async forceRefreshCrash() {
        sb.rpc('advance_crash_round'); // на всякий случай подталкиваем раунд тоже
        const { data: csArr } = await sb.rpc('get_crash_state');
        const cs = csArr && csArr[0];
        if (cs) { this.lastCrashSig = null; this.onCrashUpdate(cs); }
    },
